...
from app.api.dependencies.current_user import current_user
...
async def task_my_list(db_session: DBSessionDep, user: UserRead = Depends(current_user)):
...
@api_tasks_router.post("/", response_model=TaskAllSubtasksReadSchema, )
...
async def take_task(db_session: DBSessionDep, task_id: int, user: UserRead = Depends(current_user), ):
...
async def start_task(db_session: DBSessionDep, task_id: int, user: UserRead = Depends(current_user), ):
    """Take task by current user"""
...
async def finish_task(db_session: DBSessionDep, task_id: int, user: UserRead = Depends(current_user), ):
    """Take task by current user"""
...
async def cancel_task(db_session: DBSessionDep, task_id: int, user: UserRead = Depends(current_user), ):
    """Take task by current user"""
...
async def abandon_task(db_session: DBSessionDep, task_id: int, user: UserRead = Depends(current_user), ):
    """Take task by current user"""
